...
If you want `AI Memory/` to live inside an Obsidian vault, use:

```bash
codex-project --obsidian-root "$HOME/Obsidian/MyVault"
```

That uses the same symlink model as cloud memory, but the destination is a vault folder.
It is the easiest way to view the memory notes directly in Obsidian.
...
- If you want the same memory across devices, sync the whole vault or use a vault-root symlink.
- If you want the safest setup, keep `.obsidian/` local and sync only markdown notes.
- Never store WebDAV, S3, Dropbox, or OneDrive credentials in memory files.
- Treat plugin config files as local secrets unless you explicitly want them synced.
